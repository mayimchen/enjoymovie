...
public class SimpleJsonAdapter extends BaseAdapter implements Filterable {
	 ...
	 *            parameter.
	 */
	public SimpleJsonAdapter(Context context, JSONArray data, int resource,
			String[] from, int[] to) {
		mData = data;
	...
	private View createViewFromResource(int position, View convertView,
		...
		View v;
		if (convertView == null) {
			v = ImageUtil.inflate(resource, parent, false);
	...
	public View getView(int position) {
		if (Log.D) {
			Log.d("size", "" + viewList.size());
		}
		return null;// viewList.get(position);
	}

	private void bindView(int position, View view) {
		JSONObject dataSet = null;
		try {
			dataSet = mData.getJSONObject(position);
		} catch (JSONException e) {
			throw new RuntimeException(e);
		}
		if (dataSet == null) {
	...
	private class SimpleFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence prefix) {
			...
			if (mUnfilteredData == null) {
				mUnfilteredData = mData;// XXX new ArrayList<Map<String,
										// ?>>(mData);
			}
			...
			if (prefix == null || prefix.length() == 0) {
				...
				JSONArray newValues = new JSONArray();// new
				...
				for (int i = 0; i < count; i++) {
					JSONObject h = null;
					try {
						h = unfilteredValues.getJSONObject(i);
					} catch (JSONException e) {
						throw new RuntimeException(e);
					}
					if (h != null) {
						...
						for (int j = 0; j < len; j++) {
							String str = null;
							try {
								str = (String) h.getString(mFrom[j]);
							} catch (JSONException e) {
								throw new RuntimeException(e);
							}
