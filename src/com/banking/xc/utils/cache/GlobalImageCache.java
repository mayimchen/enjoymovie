...
public class GlobalImageCache {
	...
	public synchronized static LruBitmapCache getLruBitmapCache() {
		if (null == lruBitmapCache) {
			lruBitmapCache = new LruBitmapCache(MyApplication.getInstance(), 60);
		}
	...
	public static void remove(BitmapDigest bitmapDigest) {
		if (Log.D) {
			Log.d(GlobalImageCache.class.getName(), "remove() bitmapDigest -->> " + bitmapDigest);
		}
	...
	public static class ImageState {
		...
		public void loading() {
			if (Log.D) {
				Log.d(GlobalImageCache.class.getName(), "loading() -->> ");
			}
		...
		public void success(Bitmap b) {
			if (Log.D) {
				Log.d(GlobalImageCache.class.getName(), "success() b -->> " + b);
			}
