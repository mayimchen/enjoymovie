...
public class BitmapUtil {
	...
	public Bitmap scaleBitmap(Bitmap b, int width, int height, boolean upsampling) {
		...
		if (Log.D) {
			Log.d(BitmapUtil.class.getName(), "scaleBitmap() width=" + width + " height=" + height + " imageWidth=" + imageWidth + " imageHeight=" + imageHeight + " upsampling=" + upsampling + " -->> ");
		}
		...
		if(//
			...
			width = DPIUtil.dip2px(ImageUtil.IMAGE_MAX_WIDTH);
			height = DPIUtil.dip2px(ImageUtil.IMAGE_MAX_HEIGHT);
		}
		...
		if (Log.D) {
			if(null != scaled){
				Log.d(BitmapUtil.class.getName(), "scaleBitmap() return width=" + scaled.getWidth() + " height=" + scaled.getHeight() + " -->> ");
			}
		}
		return scaled;
	}
	...
	private Bitmap decodeFile(File f, int suggestedSize) {
		...
		try {
			...
			if (Log.D) {
				Log.d(BitmapUtil.class.getName(), "decodeFile() scale=" + scale + " -->> ");
			}
